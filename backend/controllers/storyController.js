...
const deleteImageFile = require("../utils/lib/deleteImageFile");
const { searchHelper, paginateHelper } = require("../utils/query/queryHelpers");
...
const deleteStory = asyncErrorWrapper(async (req, res, next) => {
    ...
    Story.findOneAndRemove({ slug: slug })
        .then((story) => {
            console.log('Story removed: ', story);
        })
        .catch((err) => {
            console.error(err);
